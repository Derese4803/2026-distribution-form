...
def main():
    ...
    if st.sidebar.button("📝 Registration Form / መመዝገቢያ ፎርም", use_container_width=True): 
        nav("Form")
    if st.sidebar.button("📊 View Data / መረጃዎችን ይመልከቱ", use_container_width=True): 
        nav("Data")
    ...
    if st.session_state["page"] == "Form":
        ...
        with st.form("main_form", clear_on_submit=True):
            ...
            def section(name, amharic, exp):
                ...
                nb = sc1.number_input(f"Beds / የ{amharic} አልጋዎች ብዛት", 0, key=f"n_{name}")
                ln = sc2.number_input(f"Length (m) / የአልጋው ርዝመት", 0.0, key=f"l_{name}")
                sk = sc3.number_input(f"Sockets Wide / የጎን ሶኬቶች ብዛት", 0, key=f"s_{name}")
                return nb, ln, sk
    ...
    # --- PAGE 2: DATA VIEW / መረጃ ---
        ...
        if check_password():
            ...
            if recs:
                ...
                final_cols = [c for c in column_order if c in df_raw.columns]
                df_ordered = df_raw[final_cols].rename(columns=mapping)

                # 3. Download Section
                c1, c2 = st.columns(2)
                c1.download_button("🖼️ Photo ZIP / ፎቶዎችን አውርድ", buf.getvalue(), "photos.zip", use_container_width=True)
                
                # utf-8-sig makes Amharic readable in Excel
                csv_data = df_ordered.to_csv(index=False).encode('utf-8-sig')
                c2.download_button("📥 Excel Data / መረጃውን በኤክሴል አውርድ", csv_data, "nursery_report.csv", "text/csv", use_container_width=True)
                ...
                for r in recs:
                    with st.container(border=True):
                        ...
                        col_t.info(f"**Remarks:** {r.general_remark}")
                        if r.photo: col_i.image(base64.b64decode(r.photo), caption="Nursery Photo")
                        if st.button(f"🗑️ Delete Record {r.id} / አጥፋ", key=f"d_{r.id}"):
